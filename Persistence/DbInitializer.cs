        ...
        public static void Initialize(AppDbContext context)
        {
            ...
            if (context.rooms.Count() == 0)
            {
                ...
                for (int i = 0; i < 20000; i++)
                {
                    ...
                    var building = random.Next(1, 20000);
                    var firstValue = random.Next(0, 20);
                    var secondValue = random.Next(firstValue, 70);
                    var desc = text.Substring(firstValue, secondValue);
            ...
            if (context.rents.Count() == 0)
            {
                ...
                for (int i = 0; i < 20000; i++)
                {   
                    var room = random.Next(1, 20000);
                    var organization = random.Next(1, 20000);
            ...
            if (context.invoices.Count() == 0)
            {
                ...
                int range = (DateTime.Today - start).Days;
                var room = random.Next(1, 20000);
                var employee = random.Next(1, 20000);
                var organization = random.Next(1, 20000);
                ...
                for (int i = 0; i < 20000; i++)
                {
                    ...
                    context.invoices.Add(new Invoice { createDate = createDate, number = value, organizationID = organization, roomID = room, puySum = random.Next(300, 1000000), payDate = payDate, employeeID = employee });
                }
                context.SaveChanges();
            }
        }
