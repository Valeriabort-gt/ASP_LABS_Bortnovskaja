...
namespace Application.Rooms.Quieres.GetRoomList
{
    public class GetRoomListQueryHandler : IRequestHandler<GetRoomListQuery, RoomListVm>
    {
        ...
        {
            var rooms = await _dbcontext.rooms.Skip((request.page) * 15).Take(15).ProjectTo<RoomLookupDto>(_mapper.ConfigurationProvider).OrderBy(record => record.id).ToListAsync(cancellationToken);
            var pagesCount = Math.Ceiling(Decimal.Parse((_dbcontext.rooms.Count() / 15.0).ToString()));
            return new RoomListVm
            {
                rooms = rooms,
                next = request.page + 1 <= pagesCount ? $"https://{request.url.Value}/room?page={request.page + 1}" : null,
                back = request.page - 1 > 0 ? $"https://{request.url.Value}/room?page={request.page - 1}" : null,
